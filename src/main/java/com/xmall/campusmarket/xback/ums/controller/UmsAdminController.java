...
import com.xmall.campusmarket.xback.ums.dto.UmsAdminLoginParam;
...
import com.xmall.campusmarket.xback.ums.model.UmsAdmin;
...
public class UmsAdminController {
    ...
    public ResponseEntity<UmsAdmin> register(@Validated @RequestBody UmsAdminParam umsAdminParam) {
        ...
        return ResponseEntity.success(umsAdmin);
    }
    ...
    public ResponseEntity login(@Validated @RequestBody UmsAdminLoginParam umsAdminLoginParam) {
        ...
        if (token == null) {
            return ResponseEntity.validateFailed("用户名或密码错误");
        }
        Map<String, String> tokenMap = new HashMap<>();
        tokenMap.put("token", token);
        tokenMap.put("tokenHead", tokenHead);
        return ResponseEntity.success(tokenMap);
    }
    ...
    public ResponseEntity refreshToken(HttpServletRequest request) {
        ...
        if (refreshToken == null) {
            return ResponseEntity.failed("token已经过期！");
        }
        Map<String, String> tokenMap = new HashMap<>();
    ...
    public ResponseEntity<CommonPage<UmsAdmin>> list(@RequestParam(value = "keyword", required = false) String keyword,
        ...
        return ResponseEntity.success(CommonPage.restPage(adminList));
    }
    ...
    @ResponseBody
    public ResponseEntity update(@PathVariable Long id, @RequestBody UmsAdmin admin) {
    ...
    public ResponseEntity updatePassword(@Validated @RequestBody UpdateAdminPasswordParam updatePasswordParam) {
        ...
        if (status > 0) {
            ...
            return ResponseEntity.failed("提交参数不合法");
        } else if (status == -2) {
            return ResponseEntity.failed("找不到该用户");
        } else if (status == -3) {
